...
$("#calendar").focusout(function () {
    $("#event-reminder").html(reminder_title);
    $("#event-reminder").show();
});
...
function refreshEvents(events_data) {
    jQuery.each(events_data, function (date, val) {
        ...
        // date = jsCalendar.tools.dateToString(date, date_format, "zh");
        calendar.unselect(date); //先清除舊有日期事件
...
let daily_id = "daily_type_1"; //記錄會員的身體狀態，預設是"生理期" (ex:生理期=daily_type_1;小產期=daily_type_2...)
